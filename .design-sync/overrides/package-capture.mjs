#!/usr/bin/env node
// forked from design-sync package-capture.mjs — settle() only awaited fonts
// + image decode, which resolves before React's post-navigation commit paints
// on components whose mount is scheduled via requestAnimationFrame (e.g.
// framer-motion's initial->animate transition). That raced the screenshot
// and produced a blank capture (backdrop only, panel still at its pre-mount
// style) even though the same component renders correctly once painted —
// confirmed by validate's non-isolated render check and by hand with an
// added wait. Fix: settle() also waits a couple of animation frames after
// networkidle so any RAF-scheduled paint has landed before the shot.
...
let pageErrs = [];
// A fixed clock (deterministic Date.now(), so date-relative preview text
// doesn't spuriously invalidate grades/hashes day to day) combined with a
// SECOND real content-bearing navigation on the SAME Page object hits a
// Playwright/Chromium bug: the second paint's requestAnimationFrame
// callbacks never fire, permanently freezing any RAF-driven mount animation
// (e.g. framer-motion's initial->animate transition) at its pre-paint state
// — confirmed by bisection (removing setFixedTime alone fixes it; so does
// giving the second navigation its own fresh Page while keeping the frozen
// clock). A brand-new page per navigation sidesteps the bug entirely, so
// every navigation below — the __dsCells discovery load AND every per-story
// capture — gets its own fresh page instead of reusing one across navigations.
async function freshPage(viewport = { width: 900, height: 700 }) {
  const p = await browser.newPage({ viewport });
  try { await p.clock.setFixedTime(new Date('2024-05-15T12:00:00Z')); } catch { /* older playwright */ }
  p.on('pageerror', (e) => pageErrs.push(String(e).split('\n')[0]));
  return p;
}
...
async function settle(p) {
  ...
  // Triple-rAF: let a requestAnimationFrame-scheduled mount (e.g. an
  // animation library's initial->animate commit) actually paint before the
  // fixed buffer below, instead of racing it.
...
for (const c of comps) {
  ...
  let cells = [];
  try {
    const discoveryPage = await freshPage(vp);
    try {
      await discoveryPage.goto(`http://127.0.0.1:${port}/${rel}`, { waitUntil: 'networkidle', timeout: 20_000 });
      cells = await discoveryPage.evaluate(() => Array.isArray(window.__dsCells) ? window.__dsCells.slice() : []);
    } finally {
  ...
  try {
    const sheetPage = await freshPage({ width: 1000, height: 700 });
    try {
      await sheetPage.goto(`http://127.0.0.1:${port}/_screenshots/review/.sheet-${c.group}__${c.name}.html`, { waitUntil: 'networkidle', timeout: 15_000 });
      await sheetPage.evaluate(() => Promise.all([...document.images].map((i) => i.decode().catch(() => {}))));
      await sheetPage.screenshot({ path: join(shotBase, `${c.group}__${c.name}.png`), fullPage: true });
    } finally {
