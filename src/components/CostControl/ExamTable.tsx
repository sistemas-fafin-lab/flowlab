...
const ExamTable: React.FC<ExamTableProps> = ({
  ...
  const allSelected = exams.every(e => selectedIds.has(e.id));
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-sm">
          <thead className="bg-slate-50 dark:bg-gray-900/40 text-[11px] uppercase tracking-wider text-slate-500 dark:text-slate-400">
            <tr>
              <th className="px-5 py-3 text-left font-bold w-10">
                <input
                  type="checkbox"
                  checked={allSelected}
          ...
          <tbody className="divide-y divide-gray-100 dark:divide-gray-700/60">
            {exams.map(exam => (
              <tr
                ...
                className={`transition-colors ${
                  selectedIds.has(exam.id)
                    ? 'bg-blue-100/70 dark:bg-blue-500/[.12]'
              ...
              >
                ...
                <td className="px-5 py-3.5">
                  <span className="font-mono text-xs text-slate-500 dark:text-slate-400">{exam.tuss}</span>
                </td>
                <td className="px-5 py-3.5">
