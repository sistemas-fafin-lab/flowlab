...
export function CuradoriaDrawer({ id, canManage, onFechar }: CuradoriaDrawerProps) {
  ...
  return createPortal(
    <div className="fixed inset-0 z-[60] flex justify-end">
      ...
      <div className="relative flex h-full w-full animate-slide-in-right flex-col border-l border-slate-200/50 bg-white/90 shadow-2xl backdrop-blur-2xl dark:border-white/10 dark:bg-gray-800/85 sm:max-w-xl lg:max-w-4xl">
        <div className="flex items-center justify-between gap-3 border-b border-gray-100 px-4 py-4 dark:border-white/10 sm:px-6">
          <div className="min-w-0">
            <h2 className="truncate text-lg font-semibold text-gray-800 dark:text-slate-100">Curadoria da cortesia</h2>
            {data?.nomePacienteLis && (
              <p className="truncate text-sm text-gray-500 dark:text-slate-400">{data.nomePacienteLis}</p>
            )}
          </div>
          <button
            type="button"
