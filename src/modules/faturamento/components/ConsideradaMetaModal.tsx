...
// setor — só as fontes marcadas aqui contam nas tabelas e no dashboard do
// módulo Faturamento; o resto (particulares, clínicas parceiras etc.) fica de
// fora até alguém marcar. Whitelist, não blacklist: fonte nova/desconhecida
// entra desmarcada. Mesmo esqueleto de ClinicasParceirasModal (issue 16) e
// RegraNfModal (issue 31): a lista vem de `operadoras`, já carregada pela
// tela de Títulos.
...
const ConsideradaMetaModal: React.FC<Props> = ({ aberto, onFechar, operadoras, onAlternar }) => {
  ...
  const filtradas = useMemo(() => {
    ...
    return [...lista].sort((a, b) => {
      if (a.consideradaMeta !== b.consideradaMeta) return a.consideradaMeta ? -1 : 1;
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
      <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl w-full max-w-lg max-h-[85vh] flex flex-col">
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 dark:border-gray-700">
          <div>
            <h2 className="text-lg font-semibold text-gray-900 dark:text-gray-100">Fontes consideradas na meta</h2>
            <p className="text-xs text-gray-500 dark:text-gray-400">
              Marque as fontes pagadoras que devem contar nas tabelas e no dashboard de Faturamento
            </p>
          </div>
          <button
            type="button"
...
export default ConsideradaMetaModal;
