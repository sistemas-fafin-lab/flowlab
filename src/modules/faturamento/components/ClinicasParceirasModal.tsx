...
// apLIS cadastra como fonte pagadora comum — a mesma tabela `fatinstituicao`
// de um convênio de saúde, sem nenhuma flag que distinga uma coisa da outra.
//
// A lista vem de `operadoras` (prop, já carregada pela tela de Títulos), não
// de uma busca própria ao apLIS: é o mesmo espelho que "Sincronizar
// operadoras" alimenta, então uma clínica que ainda não apareceu aqui só
// precisa daquele botão, não de um caminho de busca paralelo.
...
const ClinicasParceirasModal: React.FC<Props> = ({ aberto, onFechar, operadoras, onAlternar }) => {
  ...
  const filtradas = useMemo(() => {
    ...
    return [...lista].sort((a, b) => {
      if (a.isClinicaParceira !== b.isClinicaParceira) return a.isClinicaParceira ? -1 : 1;
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
      <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl w-full max-w-lg max-h-[85vh] flex flex-col">
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 dark:border-gray-700">
          <div>
            <h2 className="text-lg font-semibold text-gray-900 dark:text-gray-100">Clínicas parceiras</h2>
            <p className="text-xs text-gray-500 dark:text-gray-400">
              Marque as fontes pagadoras que são clínicas parceiras, não convênios de saúde
            </p>
          </div>
          <button
            type="button"
...
export default ClinicasParceirasModal;
