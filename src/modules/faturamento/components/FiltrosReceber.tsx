...
import { Bookmark, Check, ChevronDown, Filter, Plus, Search, Trash2, X } from 'lucide-react';
...
// ─── Views salvas ──────────────────────────────────────────────────────────────
// Um recorte de filtros que o operador monta com frequência (ex.: "AMIL em
// atraso") e não quer remontar toda vez. Aplicar é imediato ao clicar — salvar
// é a única ação que precisa de confirmação (dar um nome). Não usa portal como
// os controles do modal: esta barra não mora num ancestral com overflow que a
// recortaria.

interface ViewsSalvasMenuProps {
  filtros: DashboardReceberFiltros;
  onAplicar: (filtros: DashboardReceberFiltros) => void;
}

const ViewsSalvasMenu: React.FC<ViewsSalvasMenuProps> = ({ filtros, onAplicar }) => {
  const { views, loading, salvar, excluir } = useViewsSalvas<DashboardReceberFiltros>('dashboard');
  const [aberto, setAberto] = useState(false);
  ...
  const aplicar = (filtrosDaView: DashboardReceberFiltros) => {
  ...
  return (
    <>
      <div className={`${VIDRO} rounded-3xl px-4 py-3 flex flex-wrap items-center gap-x-3 gap-y-2`}>
        ...
        <ViewsSalvasMenu filtros={filtros} onAplicar={onFiltrar} />
