...
> Não há mais geração/materialização de slots: o `get-disponibilidade` calcula a agenda dos próximos N dias (env `DISPONIBILIDADE_DIAS`, default 365) a partir da base + exceções, no fuso de Brasília (`AGENDA_TZ_OFFSET`, default `-03:00`).
