...
- **Quais departamentos entram no consumo em 2 etapas (§2.7/§5)** — ✅ resolvido: apenas **Biologia Molecular** (migration `20260701140000_fase5_controla_consumo_biomol.sql`), testado em test. Enquanto a decisão esteve aberta, todos ficavam no `out` direto e nada quebrava — nunca foi bloqueante da Parte A/B; era só a configuração dos setores opt-in. (A antiga dúvida "origem da baixa: principal vs. escolhida" também fica resolvida: **default = principal**, com o ramo transfer/out decidido pelo `department`.)
