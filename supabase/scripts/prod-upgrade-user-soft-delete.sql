-- ═══════════════════════════════════════════════════════════════════════════════
-- UPGRADE DE PROD — Remoção de usuário (soft delete) + drift pendente
-- Arquivo: supabase/scripts/prod-upgrade-user-soft-delete.sql
-- Gerado em 2026-07-21
-- ═══════════════════════════════════════════════════════════════════════════════
--
-- COMO RODAR: cole o arquivo INTEIRO no SQL Editor do Supabase e execute de uma vez.
-- Roda em transação única — se qualquer passo falhar, nada é aplicado.
-- É idempotente: pode ser reexecutado sem efeito colateral.
--
-- CONTEÚDO:
--   Parte 1 — 20260624030000_add_function_quantity_to_it_requests
--             (coluna ausente em prod; ITTaskDrawer.tsx grava nela)
--   Parte 2 — 20260721120000_user_soft_delete
--             (soft delete de usuários; + check_user_dependencies)
--   Parte 3 — Conferência final (não altera nada; devolve um checklist)
--
-- Depois de aplicar, remover um cadastro é:
--   SELECT soft_delete_user_by_cpf('03707582183');
--
-- ═══════════════════════════════════════════════════════════════════════════════
...
-- ═══════════════════════════════════════════════════════════════════════════════
-- PARTE 1 — it_requests.function_quantity  (migration 20260624030000)
-- ═══════════════════════════════════════════════════════════════════════════════
...
-- ═══════════════════════════════════════════════════════════════════════════════
-- PARTE 2 — Soft delete de usuários  (migration 20260721120000)
-- ═══════════════════════════════════════════════════════════════════════════════
...
-- ═══════════════════════════════════════════════════════════════════════════════
-- PARTE 2b — Diagnóstico de dependências (de supabase/scripts/remover_usuario.sql)
-- ═══════════════════════════════════════════════════════════════════════════════
...
-- ═══════════════════════════════════════════════════════════════════════════════
-- PARTE 3 — CONFERÊNCIA (somente leitura; devolve o checklist como resultado)
-- ═══════════════════════════════════════════════════════════════════════════════
-- Só consulta catálogo: não falha se algum objeto não existir.
-- Os dois últimos itens olham CORPO de função — é o ponto cego que a comparação
-- de schema via API não cobre (foi assim que o cutover da Fase 5 passou batido).

SELECT * FROM (

  -- colunas esperadas
  SELECT '1. coluna' AS bloco,
         v.t || '.' || v.c AS item,
         CASE WHEN EXISTS (
  ...
  FROM (VALUES
    ...
    ('user_profiles','deleted_snapshot'),
    ('user_profiles','is_active'),
    ('it_requests','function_quantity')
  ) v(t,c)

  UNION ALL
  -- is_active precisa ser coluna gerada
  SELECT '2. coluna gerada', 'user_profiles.is_active = (deleted_at IS NULL)',
         COALESCE((
           SELECT CASE WHEN is_generated = 'ALWAYS' THEN 'OK' ELSE '>>> NÃO É GERADA' END
             FROM information_schema.columns
            WHERE table_schema='public' AND table_name='user_profiles' AND column_name='is_active'
         ), '>>> AUSENTE')

  UNION ALL
  -- view com security_invoker (sem isso ela ignora o RLS e vaza para anon)
  SELECT '3. view', 'active_user_profiles (security_invoker)',
         COALESCE((
           SELECT CASE WHEN c.reloptions::text ILIKE '%security_invoker=%true%'
                       THEN 'OK' ELSE '>>> SEM security_invoker' END
             FROM pg_class c JOIN pg_namespace n ON n.oid = c.relnamespace
            WHERE n.nspname='public' AND c.relname='active_user_profiles' AND c.relkind='v'
         ), '>>> AUSENTE')

  UNION ALL
  -- funções da remoção
  SELECT '4. função', v.sig,
         CASE WHEN to_regprocedure(v.sig) IS NULL THEN '>>> AUSENTE' ELSE 'OK' END
  FROM (VALUES
    ('public.soft_delete_user(uuid)'),
    ('public.soft_delete_user_by_cpf(text)'),
    ('public.restore_user(uuid)'),
    ('public.list_deleted_users()'),
    ('public.assert_can_manage_users()'),
    ('public.check_user_dependencies(uuid)')
  ) v(sig)

  UNION ALL
  -- anon não pode executar as funções destrutivas
  SELECT '5. permissão', 'anon NÃO executa ' || v.sig,
         CASE
           WHEN to_regprocedure(v.sig) IS NULL THEN '>>> AUSENTE'
           WHEN has_function_privilege('anon', to_regprocedure(v.sig), 'EXECUTE')
                THEN '>>> ANON PODE EXECUTAR'
           ELSE 'OK'
         END
  FROM (VALUES
    ('public.soft_delete_user(uuid)'),
  ...
  UNION ALL
  -- CORPO DE FUNÇÃO: cutover da Fase 5 (legado debita products.quantity direto)
  SELECT '6. corpo de função', 'update_stock_on_movement usa product_stock',
         COALESCE((
           SELECT CASE WHEN pg_get_functiondef(to_regprocedure('public.update_stock_on_movement()')) ILIKE '%product_stock%'
                       THEN 'OK (cutover Fase 5 aplicado)'
                       ELSE '>>> CORPO LEGADO — cutover da Fase 5 NÃO aplicado' END
           WHERE to_regprocedure('public.update_stock_on_movement()') IS NOT NULL
         ), 'n/a (função não existe)')

  UNION ALL
  -- Triggers de criação de perfil em auth.users (dois = signup quebra por PK duplicada)
  SELECT '7. trigger', 'triggers em auth.users -> create_user_profile',
         (SELECT CASE count(*)
                   WHEN 0 THEN 'nenhum (perfil criado pelo client, com CPF)'
                   WHEN 1 THEN 'ATENÇÃO: 1 trigger — client tenta inserir de novo e o perfil fica sem CPF'
                   ELSE '>>> ' || count(*) || ' TRIGGERS — signup quebra por PK duplicada'
                 END
            FROM pg_trigger tg
           ...
           WHERE n.nspname='auth' AND c.relname='users'
             AND p.proname='create_user_profile' AND NOT tg.tgisinternal)

  UNION ALL
  -- Quantos usuários já estão removidos
  SELECT '8. dados', 'usuários removidos (soft delete)',
         (SELECT count(*)::text FROM user_profiles WHERE deleted_at IS NOT NULL)
...
-- ═══════════════════════════════════════════════════════════════════════════════
-- Qualquer linha com ">>>" precisa de atenção antes de usar a remoção de usuário.
-- ═══════════════════════════════════════════════════════════════════════════════
