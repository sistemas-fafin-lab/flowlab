// api/_lib/handlers/qualidade-buscar-pii-cortesias.ts
// Ação `buscar-pii-cortesias` — nome do paciente por `codRequisicao`, em
// lote, para a listagem de Cortesias (src/modules/qualidade/cortesias.ts,
// `buscarNomesPacientes`). PII sob demanda (P10): nunca persistida em
// `qa_cortesias`, só devolvida nesta resposta. `periodo`/`recorte` do corpo
// não estreitam a consulta ao LIS (o filtro real já é `codigosRequisicao`,
// vindo da listagem já lida de `qa_cortesias`) — aceitos só para manter o
// mesmo contrato de `chamarQualidadeApi`.
...
export default async function handler(req: VercelRequest, res: VercelResponse): Promise<void> {
  ...
  try {
    ...
    console.error('[qualidade/buscar-pii-cortesias] erro:', describeError(err));
